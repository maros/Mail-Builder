...
ok($image = Mail::Builder::Image::File->new('t/testfile.gif'),'Create simple object');
isa_ok ($image, 'Mail::Builder::Image');
isa_ok ($image, 'Mail::Builder::Image::File');
...
eval {
	$image = Mail::Builder::Image::File->new('t/missingfile.gif');
};
like($@,qr/Could not find\/open file/);
eval {
	$image = Mail::Builder::Image::File->new('t/testfile.txt');
};
